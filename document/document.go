...
func (d *Document) buildAllPages(ctx context.Context) (all []*Page, structTags []layout.StructTagInfo, err error) {
	...
	if d.header != nil || d.footer != nil {
		total := len(all)
		for i, p := range all {
			pctx := PageContext{PageIndex: i, TotalPages: total}
			if d.header != nil {
				d.header(pctx, p)
			}
			if d.footer != nil {
	...
	if d.headerElem != nil || d.footerElem != nil {
		...
		for i, p := range all {
			...
			ps := p.effectiveSize()
			if d.headerElem != nil {
				d.renderElementDecorator(d.headerElem, pctx, p, d.margins.Left, ps.Height-d.margins.Top, headerHeight)
			}
			if d.footerElem != nil {
				d.renderElementDecorator(d.footerElem, pctx, p, d.margins.Left, d.margins.Bottom+footerHeight, footerHeight)
			}
		}
	}
...
// [Writer]. The zero value reproduces [Document.WriteTo]. It is equivalent
// to WriteToWithContext with a background context.
func (d *Document) WriteToWithOptions(w io.Writer, opts WriteOptions) (int64, error) {
	return d.WriteToWithContext(context.Background(), w, opts)
}

// WriteToWithContext is the context-aware variant of WriteToWithOptions. It
// checks ctx at page boundaries during layout and at object boundaries
// during serialization, returning ctx.Err() (context.Canceled or
// context.DeadlineExceeded) if the context is done. This lets callers bound
// a render with a real deadline rather than abandoning a goroutine that
// keeps working. On cancellation no bytes are guaranteed to have been
// written; treat the output as incomplete.
func (d *Document) WriteToWithContext(ctx context.Context, w io.Writer, opts WriteOptions) (int64, error) {
	if err := ctx.Err(); err != nil {
		return 0, err
	}
	allPages, structTags, err := d.buildAllPages(ctx)
	if err != nil {
		return 0, err
	}
